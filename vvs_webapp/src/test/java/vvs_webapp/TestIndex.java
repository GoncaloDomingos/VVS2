...
//		}
	...
	@Test
	public void updateCustomerContactsTest() throws IOException {
		// get a specific link
		HtmlAnchor addCustomerLink = page.getAnchorByHref("updateCustomerContacts.html");
		// click on it
		HtmlPage nextPage = (HtmlPage) addCustomerLink.openLinkInNewWindow();
		// check if title is the one expected
		assertEquals("Enter Name", nextPage.getTitleText());
		...
		HtmlInput phoneInput = addCustomerForm.getInputByName("phone");
		phoneInput.setValueAttribute("123456799");
		// submit form
		...
		HtmlAnchor getCustomersLink = page.getAnchorByHref("GetAllCustomersPageController");
		HtmlPage nextPageOAlexechato = (HtmlPage) getCustomersLink.openLinkInNewWindow();

		final HtmlTable table = nextPageOAlexechato.getHtmlElementById("clients");
		System.out.println("---------------------------------");
		for (final HtmlTableRow row : table.getRows()) {
			System.out.println("Found row");
			for (final HtmlTableCell cell : row.getCells()) {
	...
	@Test
	public void addNewSale() throws IOException {
		String VATNUMBER = "197672337";
		
		for(int i = 0; i < 2; i++)
			createSale();

		HtmlAnchor closeSale = page.getAnchorByHref("UpdateSaleStatusPageControler");
		HtmlPage nextPage = (HtmlPage) closeSale.openLinkInNewWindow();

		HtmlForm oQueQuiseres = nextPage.getForms().get(0);
		HtmlInput input = oQueQuiseres.getInputByName("id");
		input.setValueAttribute("3");

		HtmlInput submit = oQueQuiseres.getInputByValue("Close Sale");
		submit.click();
	}

	private void createSale() throws IOException {
		String VATNUMBER = "197672337";

		HtmlAnchor addSale = page.getAnchorByHref("addSale.html");
		HtmlPage nextPage = (HtmlPage) addSale.openLinkInNewWindow();
		...
		assertTrue(htmlPage.asText().contains(VATNUMBER));
	}
